...
public class CSVConverter implements RecordConverter<CSVRecord> {
    ...
    protected CSVConverter(RecordBuilderFactory recordBuilderFactory, boolean useHeader, CSVFormat csvFormat) {
    ...
    public static CSVFormat createCSVFormat(char fieldDelimiter, String recordDelimiter, String textEnclosure,
        ...
        if (textEnclosureCharacter != null) {
            format = format.withQuote(textEnclosureCharacter).withQuoteMode(QuoteMode.ALL);
        } else {
    ...
    public Schema inferSchema(CSVRecord record) {
        ...
        for (int i = 0; i < record.size(); i++) {
            ...
            String finalName = SchemaUtils.getCorrectSchemaFieldName(fieldName, index++, existNames);
