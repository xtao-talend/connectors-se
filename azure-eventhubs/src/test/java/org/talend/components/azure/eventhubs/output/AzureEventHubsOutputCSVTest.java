...
class AzureEventHubsOutputCSVTest extends AzureEventHubsRWTestBase {
    ...
    protected static final String EVENTHUB_NAME = "eh-junit-out-csv";
    ...
    void testSimpleSend() {
        ...
        for (int index = 0; index < PARTITION_COUNT; index++) {
            ...
            records.stream().forEach(record -> expectStringList
                    .add(record.toString().replaceAll("\"Id\"", "\"field0\"").replaceAll("\"Name\"", "\"field1\"")));
    ...
    void testFirstColumnEmpty() {
        ...
        for (int index = 0; index < PARTITION_COUNT; index++) {
            ...
            for (int i = 0; i < RECORD_PER_PARTITION; i++) {
                ...
                records.add(record);
                expectStringList.add(record.toString().replaceAll("\"Id\"", "\"field0\"").replaceAll("\"Name\"", "\"field1\""));
            }
            final String config = configurationByExample().forInstance(outputConfiguration).configured().toQueryString();
            getComponentsHandler().setInputData(records);
            Job.components().component("emitter", "test://emitter")
    ...
    void testAllType() {
        ...
        expectStringList.add("AvroRecord{delegate={\"field0\": \"test_string_" + uniqueId
                + "\", \"field1\": \"false\", \"field2\": \"0.25\", \"field3\": \"0.25\","
                + " \"field4\": \"1000\", \"field5\": \"100\", \"field6\": \"2019-11-15T17:49:05.251Z[UTC]\", \"field7\": \"test_bytes\"}}");
        expectStringList.add("AvroRecord{delegate={\"field0\": \"test_string_" + uniqueId
